...
class GroupNode(Serializable, QGraphicsRectItem):
    ...
    def calculateBoundingBox(self) -> QRectF:
        ...
        :return: bounding rectangle for all child nodes
        """
        ...
        # Add padding around nodes and account for title bar
        padding = 20
        group_pos = self.pos()
        
        # Ensure nodes don't overlap the title bar
        # Title bar is at the top, so min_y should be at least title_bar_height below group top
        min_y_with_header = max(min_y, group_pos.y() + self._title_bar_height + padding)
        
        return QRectF(min_x - padding, min_y_with_header - padding,
                      max_x - min_x + 2 * padding, max_y - min_y_with_header + 2 * padding)
    ...
    def collapse(self) -> None:
        ...
        # Hide all edges connected to child nodes (edges are on sockets, not nodes)
        for node in self.child_nodes:
    ...
    def expand(self) -> None:
        ...
        # Show all edges connected to child nodes (edges are on sockets, not nodes)
        for node in self.child_nodes:
            for socket in node.inputs + node.outputs:
                for edge in socket.edges:
                    if edge.grEdge is not None:
