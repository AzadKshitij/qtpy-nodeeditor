...
"""
from qtpy.QtWidgets import QGraphicsView, QApplication, QWidget, QGraphicsItem
...
STATE_STRING = ['', 'Noop', 'Edge Drag',
                'Edge Cut', 'Edge Rerouting', 'Node Drag']
...
class QDMGraphicsView(QGraphicsView):
    ...
    def __init__(self, grScene: 'QDMGraphicsScene', parent: QWidget = None) -> None:
        """
        ...
        # edge snapping
        self.snapping = EdgeSnapping(
            self, snapping_radius=EDGE_SNAPPING_RADIUS)
    ...
    def leftMouseButtonPress(self, event: QMouseEvent) -> None:
        ...
        if hasattr(item, "node"):
            ...
            if self.mode == MODE_NOOP:
                ...
                if DEBUG_EDGE_INTERSECT:
                    print(">> edgeIntersect start:",
                          self.edgeIntersect.draggedNode)
        ...
        if self.mode == MODE_EDGE_DRAG and item is not None:
    ...
    def mouseMoveEvent(self, event: Optional[QMouseEvent]) -> None:
        ...
        try:
            ...
            if self.isSnappingEnabled(event):
                _, scenepos = self.snapping.getSnappedToSocketPosition(
                    scenepos)
            if modified:
                self.update()
    ...
    def setSocketHighlights(self, scenepos: QPointF, highlighted: bool = True, radius: float = 50):
        ...
        # Get all items and filter for sockets
        scene_items = self.grScene.items(scanrect)
        socket_items: List[QDMGraphicsSocket] = [
            item for item in scene_items if isinstance(item, QDMGraphicsSocket)
        ]

        # Set highlight state
        for grSocket in socket_items:
    ...
    def getItemAtClick(self, event: QMouseEvent) -> Optional['QGraphicsItem']:
    ...
    def zoomIn(self):
        ...
        return self.zoomInFactor

    def zoomOut(self):
        # self.scale(1 / self.zoomInFactor, 1 / self.zoomInFactor)
        self.zoom -= self.zoomStep
        return 1 / self.zoomInFactor

    def applyZoom(self, zoomFactor: float) -> None:
        clamped = False
    ...
    def wheelEvent(self, event: Optional[QWheelEvent]) -> None:
        ...
        if event.modifiers() & Qt.KeyboardModifier.ControlModifier:
            ...
            if event.angleDelta().y() > 0:
                zoomFactor = self.zoomIn()
            else:
                zoomFactor = self.zoomOut()

            self.applyZoom(zoomFactor)
